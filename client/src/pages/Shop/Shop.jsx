...
export default function Shop() {
  const { loading } = useContext(AuthContext);
  const [books, setBooks] = useState([]);
  const [showPaymentPage, setShowPaymentPage] = useState(false); // State to toggle payment page
  ...
  // Render payment page if showPaymentPage is true
  if (showPaymentPage) {
    return <PaymentPage />;
  }
      ...
      <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-8">
        {books.map(book => (
          <Card key={book.id}>
            ...
            <button
              onClick={handleBuyNow} // Call handleBuyNow function on button click
